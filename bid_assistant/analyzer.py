...
def _analysis_prompt(document: ParsedDocument) -> str:
    text = document.full_text[:60000]
...
def analyze_document(
    ...
    *,
    ...
    try:
        payload = client.chat_json([{"role": "user", "content": _analysis_prompt(document)}])
        analysis = TenderAnalysis.model_validate({**payload, "analysis_mode": "llm", "warnings": document.warnings})
        _validate_source_quotes(analysis, document)
        analysis.outline = build_outline(analysis)
        if len(document.full_text) > 60000:
            analysis.warnings.append("文件超过 60000 字，本次 LLM 分析只处理了前部内容。")
        return analysis
