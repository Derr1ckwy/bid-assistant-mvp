...
with tab_knowledge:
    ...
    st.caption("切换类别只改变当前查看和上传位置，不会删除其他类别的资料。")
    knowledge_upload_nonce_key = f"knowledge_upload_nonce_{current_id}_{category}"
    knowledge_uploads = st.file_uploader(
        "上传资料",
        type=[extension.lstrip(".") for extension in sorted(KNOWLEDGE_EXTENSIONS)],
        accept_multiple_files=True,
        key=(
            f"knowledge_upload_{current_id}_{category}_"
            f"{st.session_state.get(knowledge_upload_nonce_key, 0)}"
        ),
    )
    if st.button(
        "保存资料",
        disabled=not knowledge_uploads,
        key=f"save_knowledge_{current_id}_{category}",
    ):
        valid_uploads = []
        ...
        if valid_uploads:
            ...
            st.session_state[knowledge_upload_nonce_key] = (
                st.session_state.get(knowledge_upload_nonce_key, 0) + 1
            )
    ...
    current_paths = files_by_category.get(category, [])
    st.markdown(f"#### {CATEGORY_LABELS[category]}（{len(current_paths)}）")
    pending_delete_key = f"pending_knowledge_delete_{current_id}_{category}"
    if current_paths:
        for path in current_paths:
            reference = f"{category}/{path.name}"
